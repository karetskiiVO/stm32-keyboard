...
USB_STRING(USB_StringManufacturingDescriptor, u"@kar_vladi"); //Vendor
USB_STRING(USB_StringProdDescriptor, u"Aboba_Keyboard"); //Product
...
void HIDDevice::stddescr (uint16_t descr, const void **data, uint16_t *size) {
    switch (descr & 0xFF00) {
...
char HIDDevice::ep0_in (config_pack_t* req, void** data, uint16_t* size) {
    if (req->bmRequestType == (USB_REQ_INTERFACE | 0x80) &&
        req->bRequest      == GET_DESCRIPTOR             &&
        req->wValue        == HID_REPORT_DESCRIPTOR) {
...
static void outp_req (uint8_t epnum) {
    uint8_t buf[10];
    usb_ep_read(INTR_NUM, (uint16_t*)&buf);
...
void HIDDevice::poll () {
  if(GPI_ON( LBTN )){
    ...
    usb_ep_write(INTR_NUM | 0x80, (uint16_t*)&report_tablet, sizeof(report_tablet));
    delay(1000000);
  ...
  if(GPI_ON( JBTN )){
    ...
    usb_ep_write(INTR_NUM | 0x80, (uint16_t*)&report_kbd, sizeof(report_kbd));
    delay(1000000);
